...
export default function WorldleFeed({
  filteredWorldleScores,
  loading,
  setWorldleCardsLoaded,
  worldleCardsLoaded,
  max,
}) {
  return (
    <Container sx={{ mb: "100px" }} maxWidth="lg">
      ...
      {loading ? (
        ...
        <>
          {filteredWorldleScores &&
            filteredWorldleScores?.map((dateObject) => (
              <div key={guid()}>
                <Typography
                  variant="h6"
                  align="center"
          ...
          >
            {max > filteredWorldleScores.length ? (
              <Button
                color="success"
                onClick={() => setWorldleCardsLoaded(worldleCardsLoaded + 5)}
                variant="contained"
              >
                Load 5 More
              </Button>
