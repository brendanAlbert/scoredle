...
export default function StatsModal({
    ...
    const myStats = useMemo(() => {
        const myScoresArray = scores
            .map((dateObject) => {
                const getWordleCrownWinner = crownify(
                    dateObject.scores,
                    "wordle"
                );
                const getWorldleCrownWinner = crownify(
                    dateObject.scores,
                    "worldle"
                );
                const getStateleCrownWinner = crownify(
                    dateObject.scores,
                    "statele"
                );
        ...
        myScoresArray
            ...
            .forEach((scoreObj) => {
                if (scoreObj?.wordScore?.length === 6) {
                    if (
                        scoreObj?.wordScore[5]?.filter((x) => x == 2).length ===
                        5
                    ) {
                        // got it in 6 guesses
                    ...
                    if (scoreObj?.wordScore) {
                        statsObject.wordleScoreDistribution[
                            scoreObj?.wordScore?.length
                        ]++;
                    }
                }
                ...
                if (scoreObj?.worldScore?.length === 6) {
                    ...
                    if (
                        scoreObj?.worldScore[5]?.filter((x) => x == 2)
                            .length === 5
                    ) {
                        // got it in 6 guesses
                ...
                if (scoreObj?.stateScore?.length === 6) {
                    // check if the last row is all 2s or else
                    if (
                        scoreObj?.stateScore[5]?.filter((x) => x == 2)
                            .length === 5
                    ) {
                        // got it in 6 guesses
                ...
                if (
                    scoreObj.stateCrown &&
                    scoreObj.stateCrown === user?.given_name
                ) {
