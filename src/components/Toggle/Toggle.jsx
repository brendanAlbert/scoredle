...
const MaterialUISwitch = styled(Switch)(({ theme, checked }) => ({
  ...
  "& .MuiSwitch-thumb": {
    ...
    "&:before": {
      ...
      //   backgroundImage: `url('data:image/svg+xml;utf8,<svg width="24" height="24" viewBox="0 0 24 24" stroke="${encodeURIComponent(
      //     "#fff"
      //   )}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"><circle cx="12" cy="8" r="5" /><path fill="${encodeURIComponent(
      //     "#fff"
      //   )}" d="M3,21 h18 C 21,12 3,12 3,21"/></svg>')`,
    },
  },
  "& .MuiSwitch-track": {
    opacity: 1,
...
export default function ScoredleToggle({ toggleState, setToggleState }) {
  const handleToggle = () => {
    // setToggleState({
    //   wordle: !toggleState.wordle,
    //   worldle: !toggleState.worldle,
    // });
